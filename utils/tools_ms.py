...
def mask_fill(mask, data, num):
    select = paddle.nn.functional.where
    replace_tensor = paddle.ones(data.shape, dtype='float32')
    replace_tensor[:] = num
    return select(mask, replace_tensor, data.astype('float32'))
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, path):
        ...
        paddle.save(model.state_dict(), path+'/'+'checkpoint.pdparams')
